  ...
  return (
    <div className="flex justify-center w-full h-full">
      <div className="flex flex-col items-center justify-center gap-4 mb-8 min-w-80">
        ...
        <form className={"w-full flex flex-col gap-6"} onSubmit={(e) => handleOnSubmit(e)}>
          <>
            <Input
              label="이메일"
              type="email"
        ...
        <div className="relative w-full mt-5 border-t border-gray-200 pt-9">
          <p className="absolute text-sm text-center text-gray-500 -top-3 min-w-16 bg-body" style={{ left: "41%" }}>
            또는
          </p>
          <Button variant="outline" onClick={() => router.push("/register")} text="회원가입" />
        </div>
      </div>
    </div>
  );
